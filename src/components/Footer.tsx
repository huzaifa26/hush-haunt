...
export default function Footer({ showFooterHeaders = true }: Props) {
    return (
        <footer className={showFooterHeaders?"flex flex-col justify-between w-[100%] bg-[#FB4A04] text-[#ffffff] h-[595px] xsm:h-auto":"flex flex-col justify-between w-[100%] bg-[#FB4A04] text-[#ffffff] h-[118px] xsm:h-auto"}>
            {
                showFooterHeaders &&
                <div className='w-[90%] m-auto flex justify-around mt-[55px] xsm:flex-col xsm:gap-[40px]'>
                    <ul>
            ...
            <div className='mb-[31px] m-auto flex flex-col items-center mt-[27px]'>
                <p className='mb-[22px] xsm:text-center min-w-[260px] w-[58.44380403458213vw]'>About | Blog | HelpCareers | Press | Investors | Security | Developers | Terms | Privacy | CA Privacy | Notice | Accessibility | Cookies</p>
                <p>© 2022 Partymode</p>
            </div>
        </footer>
    )
